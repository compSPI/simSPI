...
def apply_complex_ctf_to_exit_wave(exit_wave_f, complex_ctf):
    ...
    Parameters
    ----------
    exit_wave_f : numpy.ndarray, shape (n_pix,n_pix)
        Exit wave array.
    complex_ctf : numpy.ndarray, shape (n_pix,n_pix)
        Complex ctf array.

    Returns
    -------
    i0 : numpy.ndarray, shape (n_pix,n_pix)
        Exit wave ctf convolution (in real space).
    """
    n_pix = exit_wave_f.shape[0]
    np_to_torch = torch.tensor(exit_wave_f * complex_ctf).reshape(1, 1, n_pix, n_pix)
    i0 = torch.abs(transforms.fourier_to_primal_2D(np_to_torch))
    return i0.detach().numpy().reshape(n_pix, n_pix)
...
def apply_dqe(i0_f, dqe):
    ...
    Parameters
    ----------
    i0_f : numpy.ndarray, shape (n_pix,n_pix)
        Image with ctf applied.
    dqe : numpy.ndarray, shape (n_pix,n_pix)
        Detective quantum efficiency (in 2D).

    Returns
    -------
    i0_dqe : numpy.ndarray, shape (n_pix,n_pix)
        Exit wave with dqe applied (in real space).
    """
    n_pix = i0_f.shape[0]
    i0_dqe_f_torch = torch.tensor(i0_f * np.sqrt(dqe)).reshape(1, 1, n_pix, n_pix)
    i0_dqe = transforms.fourier_to_primal_2D(i0_dqe_f_torch).real
    return i0_dqe.detach().numpy().reshape(n_pix, n_pix)
...
def apply_poisson_shot_noise_sample(signal, dose, noise_bg=0):
    ...
    Parameters
    ----------
    signal : numpy.ndarray, shape (n_pix,n_pix)
    ...
    Returns
    -------
    shot_noise_sample : numpy.ndarray, shape (n_pix,n_pix)
...
def exit_wave_to_image(exit_wave_f, complex_ctf, dose, noise_bg, dqe, ntf):
    ...
    n_pix = i0.shape[0]
    i0_torch = torch.tensor(i0).reshape(1, 1, n_pix, n_pix)
    i0_f = (
        transforms.primal_to_fourier_2D(i0_torch).detach().numpy().reshape(n_pix, n_pix)
    )
