...
def fill_parameters_dictionary(mrc_file=None,
    ...
    dic['optics']['cond_ap_angle'] = optics_params[
        5]  # aperture angle in mrad of the beam furnished by the condenser lens
    dic['optics']['defocus_nominal'] = optics_params[6]  # nominal defocus value in um
    dic['optics']['defocus_syst_error'] = optics_params[
        7]  # standard deviation of a systematic error added to the nominal defocus, measured in um. The same error is added to the defocus of every image
    dic['optics']['defocus_nonsyst_error'] = optics_params[
        8]  # standard deviation of a nonsystematic error added to the nominal defocus and the systematic error, measured in um. A new value of the error is computed for every image
...
def define_grid_in_fov(sample_dimensions, optics_params, detector_params, pdb_file=None, Dmax=None, pad=1.):
    """
	"""
    fov_Lx, fov_Ly, boxsize = get_fov(sample_dimensions, optics_params, detector_params, pdb_file=pdb_file, Dmax=Dmax,
                                      pad=pad)
    #
...
def define_grid(sample_dimensions, pdb_file=None, Dmax=None):
    ...
    print(
        "Grid of {0} molecules that cover the volume: {1} nm --> {2} nm (step size: {3} nm)".format(numpart, -xy_origin,
                                                                                                    xy_origin,
                                                                                                    step_size))
